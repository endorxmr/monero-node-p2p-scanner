...
class Bucket:
    ...
    def get_peers(self):
        ...
        for peer in [e.entries for e in self.payload_section.entries['local_peerlist_new']]:
            ...
            if 'pruning_seed' in peer:
                m_pruning_seed = peer['pruning_seed'].value
            else:
                m_pruning_seed = None
            if 'rpc_port' in peer:
                m_rpc_port = peer['rpc_port'].value
            else:
                m_rpc_port = None
            # TODO: check if there is more than 1 rpc_port? And any other properties that may have been missed
            # if len(peer) > 2:
            #     print(peer.keys())
            # if len(peer['adr'].entries) > 2:
            #     print(peer['adr'].entries)
            # if len(peer['adr'].entries['addr'].entries) > 2:
            #     print(peer['adr'].entries['addr'].entries)

            peers.append({
                'ip': m_ip,
                'port': m_port,
                'pruning_seed': m_pruning_seed,
                'rpc_port': m_rpc_port
            })
